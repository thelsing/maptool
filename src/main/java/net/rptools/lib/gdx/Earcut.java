...
/**
 * Adapted from https://github.com/earcut4j/earcut4j and modified to use float and short instead of int and float
 */
public final class Earcut {
    ...
    private static void earcutLinked(Node ear, ShortArray triangles, int dim, float minX, float minY, float invSize, int pass) {
        if (ear == null)
            return;

        // interlink polygon nodes in z-order
        if (pass == Short.MIN_VALUE && invSize != Float.MIN_VALUE)
    ...
    private static void splitEarcut(Node start, ShortArray triangles, int dim, float minX, float minY, float size) {
    ...
    private static Node linkedList(float[] data, short start, short end, short dim, boolean clockwise) {
        Node last = null;
