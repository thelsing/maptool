...
public class FoundryModuleImporter {
  ...
  /**
   * Creates a new {@code FoundryModuleImporter} object.
   *
  ...
  private void foo() throws IOException {
    ...
    try (InputStreamReader reader = new InputStreamReader(new FileInputStream(moduleFile))) {
    ...
    String mapName = FilenameUtils.removeExtension(moduleFile.getName());
  ...
  private void importModule(Path moduleJson) throws IOException {
    try (InputStreamReader reader = new InputStreamReader(Files.newInputStream(moduleJson))) {
      ...
      for (var pack : moduleFile.getAsJsonArray("packs")) {
        importPack(pack.getAsJsonObject(), moduleJson.getParent());
      }
    }
  }

  private void importPack(JsonObject pack, Path dir) throws IOException {
    var label = pack.getAsJsonPrimitive("label").getAsString();
    var pathString = pack.getAsJsonPrimitive("path").getAsString();
    if(pathString.startsWith("/")) {
      pathString = pathString.substring(1);
    }
    var packPath = dir.resolve(pathString);
        ...
        switch (type) {
          ...
          case "pdf" -> {
            ...
            if (src.isJsonPrimitive()) {
              builder.append("<a href=\"%s\">PDF</a>".formatted(pageObject.getAsJsonPrimitive("src").getAsString()));
            }
          }
          case "video" -> {
            builder.append("<a href=\"%s\">VIDEO</a>".formatted(pageObject.getAsJsonPrimitive("src").getAsString()));
          }
          case "image" -> {
            builder.append("<img src=\"%s\"/>".formatted(pageObject.getAsJsonPrimitive("src").getAsString()));
