...
import net.rptools.maptool.model.zones.CampaignChanged;
...
public class NoteBookPanel extends JFXPanel {
  ...
  public static NoteBookPanel createMapBookmarkPanel() {
    ...
    new MapToolEventBus().getMainEventBus().register(panel);
    return panel;
  }
  ...
  private void initFX() {
    ...
    TreeTableColumn<TableTreeItemHolder, String> thirdColumn =
        new TreeTableColumn<>(I18N.getText("panel.NoteBook.pathColumn"));
    thirdColumn.setCellValueFactory(
        cellDataFeatures -> {
          TableTreeItemHolder holder = cellDataFeatures.getValue().getValue();
          if (holder instanceof NoteBookEntryTreeItem) {
            var entry = (NoteBookEntryTreeItem) holder;
            return new SimpleStringProperty(entry.getEntry().getPath());
          }
          return new SimpleStringProperty("");
        });
   ...
   */
  @Subscribe
  private void campaignChanged(CampaignChanged event) {

    if (event.newCampaign() != null) {
      Platform.runLater(
          () -> {
