...
public class TuioInputSource extends AbstractInputSource implements TuioListener {	
	 ...
	 * @param pa the pa
	 */
	public TuioInputSource(JFrame pa){
		this(pa, 3333);
	}
	 ...
	 * @param port the port
	 */
	public TuioInputSource(JFrame pa, int port){
		super(pa);
		...
		windowWidth = pa.getWidth();// MT4jSettings.getInstance().getWindowWidth();
		windowHeight = pa.getHeight();//MT4jSettings.getInstance().getWindowHeight();
	}
