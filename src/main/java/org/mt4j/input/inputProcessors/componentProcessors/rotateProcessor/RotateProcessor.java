...
public class RotateProcessor extends AbstractCursorProcessor {
	...
	public RotateProcessor(JFrame graphicsContext){
	 ...
	 */
	public RotateProcessor(JFrame graphicsContext, boolean stopEventPropagation){
	...
	public void cursorStarted(InputCursor newCursor, AbstractCursorInputEvt positionEvent) {
		Component comp = positionEvent.getTarget();
...
//				logger.debug(this.getName() + " already had 1 unused cursor - we can try start gesture! used Cursor ID:" + otherCursor.getId() + " and new cursor ID:" + newCursor.getId());
	...
	public void cursorUnlocked(InputCursor c) {
		...
		if (availableCursors.size() >= 2){ //we can try to resume the gesture
			...
			Component comp = firstCursor.getFirstEvent().getTarget();
	...
	private class RotationContext {
		 ...
		 */
		public RotationContext(InputCursor pinFingerCursor, InputCursor rotateFingerCursor, Component object){
...
//			Vector3D interPoint = getIntersection(applet, object, pinFingerCursor);
	//		Vector3D interPoint = getIntersection(pinFingerCursor.getCurrentEvent().getCurrentTarget(), pinFingerCursor);
			if (hasIntersection(pinFingerCursor.getCurrentEvent().getCurrentTarget(), pinFingerCursor))
				pinFingerNew = pinFingerCursor.getPosition();
			else{
...
//			Vector3D interPointRot = getIntersection(applet, object, rotateFingerCursor);
		//	Vector3D interPointRot = getIntersection( object, rotateFingerCursor);

			if (hasIntersection(object, rotateFingerCursor))
				rotateFingerStart = rotateFingerCursor.getPosition();
			else{
