...
public class TapAndHoldProcessor extends AbstractCursorProcessor {
	...
	private Component lastCurrentTarget;
	 ...
	 * @param pa the pa
	 */
	public TapAndHoldProcessor(JFrame pa) {
		this(pa, 1000, false);
	}
	
	public TapAndHoldProcessor(JFrame pa, int duration){
	 ...
	 */
	public TapAndHoldProcessor(JFrame pa, int duration, boolean stopPropatation) {
...
//		logger.setLevel(Level.DEBUG);
	...
	{
		TapAndHoldProcessor processor = this;
		timer = new Timer();
...
//			IMTComponent3D currentTarget = c.getCurrentEvent().getCurrentTarget(); //FIXME this will often return the wrong target since we are not in a processInputEvent() method!
			Component currentTarget = lastCurrentTarget;
